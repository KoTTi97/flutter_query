...
Future<List<String>> fetchTasks(QueryFunctionContext context) =>
    api.listTasks();

QueryOptions<List<String>> tasksQuery() => QueryOptions<List<String>>(
      queryKey: tasksKey,
      queryFn: fetchTasks,
...
Future<void> main() async {
  ...
  final tasks = await client.query(tasksQuery());
  print('fetched ${tasks.length} tasks (api calls: ${api.calls})');
  ...
  void report(QueryResult<int> result) {
    switch (result) {
      ...
      case QuerySuccess(:final data, :final isFetching):
        print('observer: $data tasks${isFetching ? ' (refetching)' : ''}');
