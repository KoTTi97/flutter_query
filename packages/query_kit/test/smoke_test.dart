...
void main() {
  group('QueryKey', () {
    test('equal keys are equal and hash alike', () {
      final a = QueryKey(<Object?>['sensors', 1]);
      final b = QueryKey(<Object?>['sensors', 1]);
      expect(a, equals(b));
    ...
    test('maps compare without regard to entry order', () {
      final a = QueryKey(<Object?>[
        'sensors',
        <String, Object?>{'room': 'kitchen', 'status': 'active'},
      ]);
      final b = QueryKey(<Object?>[
        'sensors',
        <String, Object?>{'status': 'active', 'room': 'kitchen'},
      ]);
      expect(a, equals(b));
      expect(a.hashCode, equals(b.hashCode));
    });
    ...
    test('a prefix matches, an unrelated key does not', () {
      final key = QueryKey(<Object?>['sensors', 3, 'readings']);
      expect(key.matches(QueryKey(<Object?>['sensors'])), isTrue);
      expect(key.matches(QueryKey(<Object?>['sensors', 3])), isTrue);
      expect(key.matches(QueryKey(<Object?>['sensors', 4])), isFalse);
      expect(key.matches(QueryKey(<Object?>['sensors']), exact: true), isFalse);
    });
    ...
    testFakeAsync('select narrows what the observer reports', (time) async {
      ...
      final observer = QueryObserver<Map<String, Object?>, String>(
        ...
        QueryObserverOptions<Map<String, Object?>, String>(
          queryKey: key,
          queryFn: (context) async => <String, Object?>{'name': 'kitchen'},
