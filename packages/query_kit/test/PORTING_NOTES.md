...
| B2-2 | Per-call `MutateCallbacks` passed to `mutate` / `mutateAsync` on a live `MutationController` that nothing listened to were dropped: `MutationObserver` runs them only while it has listeners | `mutateAsync(…, callbacks:)` on an unlistened controller: `onSuccess` never ran | `MutationController.mutateAsync` holds a listener on its observer for the length of the run and releases it on completion, so per-call callbacks run whenever the controller is not disposed. Options: (a) document it — an API trap with no reason; (b) keep the observer subscribed for the controller's whole life — breaks the detach-when-unlistened semantics C50 and F08/F09 pin, and the mutation's `gcTime`; (c) a per-run hold — chosen, it changes nothing outside a run. A listened controller still notifies once per change (second case). Fixture changes: F08/F09 "disposing an unlistened MutationController detaches it" now disposes **mid-run** (1 observer, then none) — an idle unlistened controller has no observer to detach; C50's two mutation-listener cases reach their first notification through a run going pending, because a run now holds the observer and no mutation settles while detached. The inside-subscribe guard stays pinned by C50's query case |
