...
| 2026-09-23 | X2 | — | Codex (gpt-6-sol, read-only) full audit after the release: no P1, nothing in the packages; REL-1 (P2, `publish.yml` did not check the tag against the pubspec) → a check step; SEC-1 (P2, actions on moving tags in an OIDC publish job) → all 24 `uses:` pinned to verified SHAs with version comments; SEC-2 (P3, example servers on every interface with open reset routes) → loopback by default, `HOST=0.0.0.0` opt-in, both READMEs say so. All three reproduced; task_manager contract 29 + Playwright 10 and showcase contract 50 green against the loopback servers. Fresh Codex pass over the fix: the check read quoted or commented pubspec values raw (P2, fixed; tested with plain, quoted, commented and pre-release versions) and the pin count said 25 for 24 (P3, fixed) |
