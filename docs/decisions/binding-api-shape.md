...
The task **detail** screen of the acceptance demo, because it is the busiest
one: one query (45 s stale time, a conditional 500 ms poll while a reminder
write is unconfirmed, seeded from the list cache), a **rename** mutation with an
optimistic patch and rollback, and a **reminder** toggle with its own. Plus, on
the header, a second query on the list key that uses `select` to count synced
tasks — same cache entry, different shape, no extra request.
...
QueryBuilder<Task>(
  ...
  builder: (context, result) => switch (result) {
    QueryPending() => const TaskDetailSkeleton(),
    QueryError(:final error, staleData: null) => ErrorBanner(error),
    QuerySuccess(:final data) || QueryError(staleData: final data!) =>
      MutationBuilder<Task, RenameInput>(
        options: renameTask(),
        builder: (context, rename) => MutationBuilder<Task, ReminderInput>(
          options: setReminder(),
          builder: (context, reminder) => TaskDetailView(
            task: data,
            onRename: (name) => rename.mutate((id: id, name: name)),
            renameError: rename.errorOrNull,
            onReminderChanged: (v) => reminder.mutate((id: id, value: v)),
...
class _TaskDetailState extends State<TaskDetail> with QueryMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final reminder = watchMutation(setReminder());

    return switch (task) { /* … flat, no builders … */ };
  }
}
```
...
class TaskDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rename = context.mutation(renameTask());
    final reminder = context.mutation(setReminder());

    return switch (task) { /* … flat, and in a StatelessWidget … */ };
  }
}
```
...
```dart
late final _task = QueryController(client, taskQuery(widget.id));
late final _rename = MutationController(client, renameTask());
late final _reminder = MutationController(client, setReminder());

@override
Widget build(BuildContext context) => ListenableBuilder(
      listenable: Listenable.merge(<Listenable>[_task, _rename, _reminder]),
      builder: (context, _) => switch (_task.result) { /* … */ },
    );
```
...
class TaskDetail extends HookWidget {
  @override
  Widget build(BuildContext context) {
    final task = useQuery(taskQuery(id));
    final rename = useMutation(renameTask());
    final reminder = useMutation(setReminder());
    return switch (task) { /* … */ };
  }
}
```
...
`QueryResult<Task> useTask(String id) => useQuery(taskQuery(id));`
...
```dart
final task   = client.observe(taskQuery(id)).asValueListenable().toSignal();
final synced = computed(() => task.value.dataOrNull?.synced ?? false);
```
