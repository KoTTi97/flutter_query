# query_kit — working notes for agents
...
not read it, at the parent's next rebuild, or at unmount. A table over
...
and a restored scope head cancelled and removed hands on again (core). A
fourth pass (V4-1–5) found that bound unbounded — a `LayoutBuilder` re-runs
its builder on a resize or a notification without its parent, so the key it
left stayed — and gave a layout builder the two proofs Flutter does give it:
changed constraints, and a rebuild one of its own reads asked for (V-B-1's
cases restored verbatim; one rebuilt by an `InheritedWidget` stays additive,
documented). The table runs seeded and polling (80 cases). Also: a failing
provider applies no online verdict, the keyed-mutation assertion compares
