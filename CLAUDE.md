...
| **`examples/showcase/`** — every feature as a screen | **done (2026-09-09, #25).** 24 screens, 172 widget tests against a dio fake of the backend and 126 Playwright end-to-end tests against the real one; a scenario-isolated dummy backend under `server/`; a contract test running the same 17 cases against fake and server. It found two library bugs no ported test could reach |
