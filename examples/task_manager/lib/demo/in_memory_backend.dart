...
///
/// Two callers, one class. The tests drive the *real* `TaskApi` — its URLs,
/// its JSON, its error normalisation and its cancellation — against this
/// instead of the express process (`test/fake_backend.dart` names it
/// `FakeBackend`). And a build with `--dart-define=QK_BACKEND=inmemory` runs
/// the app itself against it, with the server's latencies, which is how the
/// documentation site embeds the app with no server behind it (`main.dart`).
/// Same routes, same scripted failures ("fail" rejects a rename, every second
/// delete fails, a reminder write is confirmed later), so the acceptance suite
/// runs offline and deterministically. Pointing the same app at the real
