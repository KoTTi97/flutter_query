...
/// [HttpClientAdapter].
///
/// Two callers, one class. The widget tests drive the *real* `ShowcaseApi` —
/// its URLs, its JSON, its error normalisation and its cancellation — against
/// this instead of the express process (`test/fake_backend.dart` is a thin
/// wrapper that loads the seed from disk). And a build with
/// `--dart-define=QK_BACKEND=inmemory` runs the app itself against it, which
/// is how the documentation site embeds live demos with no server behind them
/// (`main.dart`). Same routes in the same order, same seed data (the
/// `server/seed.json` it is handed), same fault chain (latency, `?delay`,
/// `?fail`, scripted `failNext`, `errorRate`) and the same control routes. That
/// it really mirrors the server is what `test/backend_contract_test.dart`
/// proves — of this very class, so the demo that ships is the one checked.
///
/// No `dart:io` here: it compiles for the web.
library;
...
class InMemoryBackend implements HttpClientAdapter {
  /// A backend holding [seed] — the decoded `server/seed.json` — answering
  /// every request after [latency].
  InMemoryBackend({
    required Map<String, Object?> seed,
    this.latency = Duration.zero,
  })  : _seed = seed,
        _defaultLatency = latency {
    reset();
  }
  ...
  /// What [reset] puts [latency] back to — this backend's `DEFAULT_LATENCY`,
  /// which is zero unless a caller asked for otherwise (the demo build asks
  /// for the server's 300 ms). The server restores its own default the same
  /// way, which is why the two can differ after a reset and why every
  /// contract case sets the latency it wants.
  ...
  void reset() {
    ...
    counter = 0;
    serial = 0;
    projectCount = _seed['projectCount']! as int;
