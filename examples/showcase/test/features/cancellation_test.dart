...
void main() {
  ...
  showcaseTest('two keystrokes inside the debounce send one request',
    ...
    await tester.enterText(find.byType(TextField), 'win');
    await tester.pump(const Duration(milliseconds: 100));
    await tester.enterText(find.byType(TextField), 'wind');
    await tester.pump(const Duration(milliseconds: 300));
    await tester.pump();

    expect(cardFact('search', 'needle=wind'), findsOneWidget);
    expect(cardFact('search', 'searching=true'), findsOneWidget);
    // `win` never became a needle, so its key was never observed.
  ...
  showcaseTest(
    ...
    expect(cardFact('search', 'needle=ab'), findsOneWidget);
    ...
    // `a` matches sixteen posts; none of them is on screen, and the backend
    // answered only the second needle.
    expect(find.text('Window contact: setup guide'), findsNothing);
    expect(h.requests('GET', search), 1);
  });
