...
void main() {
  showcaseTest('every reader shows its selection after one request',
    ...
    expect(inReader('builder', 'first=Update the gateway firmware'),
        findsOneWidget);
    expect(inReader('mixin', 'done=1'), findsOneWidget);
    expect(inReader('mixin', 'open=2'), findsOneWidget);
    expect(
        inReader('controller', 'Update the gateway firmware'), findsOneWidget);
    expect(inReader('controller', 'Rename the sensors in the hallway'),
        findsOneWidget);
    expect(inReader('controller', 'Replace the battery in the window contact'),
        findsOneWidget);
