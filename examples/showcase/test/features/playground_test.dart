...
Future<void> open(WidgetTester tester, Harness h) async {
  ...
  expect(rowText(1, 'Update the gateway firmware'), findsOneWidget);
...
void main() {
  ...
  showcaseTest(
    ...
    await openEditor(tester, 2, 'Rename the sensors in the hallway');
  ...
  showcaseTest(
    ...
    await openEditor(tester, 1, 'Update the gateway firmware');
  ...
  showcaseTest(
      ...
      (tester, h) async {
    await open(tester, h);
    await openEditor(tester, 3, 'Replace the battery in the window contact');
