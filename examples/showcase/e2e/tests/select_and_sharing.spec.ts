...
test('every reader shows its selection after one request', async ({ page, open, scenario }) => {
  ...
  await expect(reader(page, 'builder').getByText('first=Update the gateway firmware', { exact: true })).toBeVisible()
  ...
  for (const text of [
    ...
    'Rename the sensors in the hallway',
    'Replace the battery in the window contact',
  ]) {
...
test('renaming todo 2 moves only the list of texts', async ({ page, open, scenario }) => {
  ...
  await expect(reader(page, 'controller').getByText('Rename the sensors in the hallway', { exact: true })).toBeHidden()
  await expect(fact(page, 'todos', 'fetchStatus=idle')).toBeVisible()
