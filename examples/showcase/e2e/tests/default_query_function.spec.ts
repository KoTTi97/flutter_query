...
test('three keyed queries fetch through the default queryFn, one request each', async ({ page, open, scenario }) => {
  ...
  await expect(text(page, '#1 Window contact: setup guide')).toBeVisible()
  ...
  await expect(text(page, 'Window contact: setup guide')).toBeVisible()
  await expect(text(page, 'Anna on "Window contact: setup guide": Worked on the first try.')).toBeVisible()
  await expect(text(page, 'Ben on "Window contact: setup guide": Connected on the second attempt.')).toBeVisible()
  await expect(text(page, 'Clara on "Window contact: setup guide": Needs a gateway restart.')).toBeVisible()
