...
```dart
client.invalidateQueries(filters: QueryFilters(queryKey: sensorsKey));
client.removeQueries(filters: QueryFilters(queryKey: sensorsKey, exact: true));
...
```dart
final sensors = client.getQueryData<List<Sensor>>(sensorsKey);
client.setQueryData<Sensor>(sensorKey(id), sensor);
client.updateQueryData<Sensor>(sensorKey(id), (previous) => previous?.copyWith(…));
client.updateQueriesData<Sensor>((previous) => …, filters: QueryFilters(…));
```
...
`int?` are two types. So are `List<Sensor>` and `List<Object?>`.
...
```dart
Future<List<Sensor>> listSensors(QueryFunctionContext context) {
  final token = CancelToken();                     // dio
  context.signal.onCancel(token.cancel);
  return dio.get<…>('/sensors', cancelToken: token).then(…);
}
```
